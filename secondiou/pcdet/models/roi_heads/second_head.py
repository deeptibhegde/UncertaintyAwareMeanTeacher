...
class SECONDHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict,is_teacher=False):
        """
        ...
        if not self.training:
            ...
            if not is_teacher:
                ...
                batch_dict['rcnn_cls_labels'] = self.forward_ret_dict['rcnn_cls_labels']

        return batch_dict

    def get_loss(self,unc_weight, tb_dict=None):
        tb_dict = {} if tb_dict is None else tb_dict
        rcnn_loss = 0
        rcnn_loss_cls, cls_tb_dict = self.get_box_iou_layer_loss(self.forward_ret_dict,unc_weight)
    ...
    def get_box_iou_layer_loss(self, forward_ret_dict,unc_weight):
        ...
        rcnn_loss_iou = (batch_loss_iou * iou_valid_mask*unc_weight).sum() / torch.clamp(iou_valid_mask.sum(), min=1.0)
